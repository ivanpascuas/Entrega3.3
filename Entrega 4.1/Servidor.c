...
int main(int argc, char *argv[])
{
	...
	for(i=0;i<7;i++){
		...
		else if (codigo == 3)
		{
			...
			for (i=0; i<strlen(nombre);i++,n--)
			{
				if (nombre[i] == nombre[n] && palindromo == 1)
				{
					palindromo = 1;
				}
				else if (nombre[i] != nombre[n])
				{
					palindromo = 0;
				}
			}
		...
		else if (codigo == 5)
		{
			...
			float trans = round(temperatura*9/5 + 32);
			sprintf(buff2, "%f,", trans);
		}
		
		else if (codigo == 6)
		{
			float temperatura = atof(p);
			float trans = round((temperatura-32)*5/9);
			sprintf(buff2, "%f,", trans);
		}
